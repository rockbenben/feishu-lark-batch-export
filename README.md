...
支持**知识库**、**云空间**和 **URL列表 TXT**，飞书和 Lark 都行。导出成 Markdown / Word / PDF，多个文件自动打包，目录结构原样保留。
...
- **URL列表 TXT** —— 选择一个每行一个 URL 的 `.txt` 文件。链接必须是当前打开的飞书 / Lark 站点，支持知识库、新旧版文档、表格、多维表格和附件链接；空行忽略，同一类型和唯一 ID 的重复链接只保留首次出现项，查询参数与锚点不影响判重

扫描「我的云空间」会一直读取到服务端明确返回最后一页；若分页游标缺失、重复或超过安全上限，会直接报错，不会把已读取的部分当成完整结果。

链接列表只增加一种输入来源：列出后仍进入同一份勾选清单，并复用原来的导出、下载、图片本地化和 zip 流程。普通文档直接从 URL 取得类型和唯一 ID，因此列出时不需要逐篇访问网页；知识库链接仍需调用现有接口把 wiki token 转成实际文档 token。列表中的普通文档暂以唯一 ID 显示，导出成功后，文件名会优先采用飞书导出结果返回的真实标题。原有导出请求会逐篇验证实际读取 / 下载权限；页面不存在、没有权限或链接格式错误会在日志中保留对应链接，其他文档继续处理。

通过 `file` URL 直接下载附件时，后台脚本会先发一个不下载正文的 `HEAD` 请求，从 `Content-Disposition` 头读取并解码原文件名，再由页面发 `GET` 下载文件；响应头缺失或无法解码时会在日志说明原因并退回唯一 ID。从知识库、文件夹或云空间列出的附件仍使用列表接口返回的原文件名，不额外发 `HEAD`。

链接列表不携带修改时间，因此「最近改过的」按 7 / 30 / 90 天批量勾选不会选中 TXT 里的文档，需要手动勾选；wiki 行要逐行调用接口转换 token，链接很多时列出会自动放慢（每行间隔 0.4 秒）。若所有链接都读取失败，会明确报错而不是列出一个空清单。
...
**多于一个文件时自动打包成一个 zip**（`飞书导出-YYYY-MM-DD.zip`），只有一个就直接下那个文件。所以不会触发 Chrome 的「允许下载多个文件」弹窗。中途点「停止」会中止当前网络请求，已经导好的部分照样打包给你。

最终产物和附件下载单次最长等待 120 秒；网络错误、HTTP 408 / 429 / 5xx 会自动重试，最多重试两次：默认间隔 1 秒，响应带 `Retry-After` 时按服务端要求等待（封顶 30 秒）。401 / 403 / 404 等确定性失败立即进入失败列表，不会重复请求。扫描和创建导出任务的 API 请求单次最多等待 30 秒；导出进度轮询不重试，总等待时间不超过 120 秒。最终失败只记录一条日志，并附上文档或附件的来源链接。

面板里有进度条和按实测均速算的剩余时间，长任务不用干等着猜。

鼠标悬停日志区域，右上角会浮现「复制」按钮，一键复制完整日志；日志里出现失败行时按钮会常驻，反馈失败链接或排查问题时不用手动选中文字。
...
| 文件名带文档唯一id | 开 | 导出的文件名带有文档的唯一id，如 `标题-doxcnAbC123.md` |

设置会记住。**序号是按目录分别计数的**：开着「保留目录结构」时每个文件夹各自从 `001` 开始，不会出现 `007`、`019` 这种跳号；平铺时才退化成全局序号。

**一并保存图片**跟格式选择相互独立：只要某篇最终产出的是 `.md`，就把图片抓到 `assets/<文档 token>/001.png` 并把链接改成相对路径。所以选「自动」时照样带图。docx / pdf / xlsx 是二进制，图片已经内嵌在文件里，这个开关对它们无意义。图片单次请求 15 秒超时，失败后间隔 1 秒重试，最多重试两次；三次都失败才在日志里记录一次错误和图片链接，Markdown 保留原链接，不会把链接改瞎。图片目录按文档 token 隔离，跟标题、序号无关：同名文档不会共用目录，同一篇文档重导多次也始终落在同一个目录。
...
- **打包要占内存。** 所有产物先收集再压 zip。内容以 Blob 形式持有（浏览器可落盘），不进 JS 堆，几百 MB 的批次没问题；但不支持 zip64，超出 ZIP32 的文件数量、名称长度、大小或偏移范围时会明确报错，不生成损坏的压缩包。zip 用 STORE 不压缩 —— docx/pdf/xlsx/png 本来就是压缩格式，再压一遍纯属白费 CPU。
- 单篇失败不中断队列，日志里记一行，跑完可以点「重试失败的 N 篇」。
- 扩展只用你浏览器里已有的登录态调飞书自己的 Web 接口，**不上传任何东西，没有服务端**。后台脚本负责转发工具栏点击、把语言文件读给面板，以及为 `file` 直链读取下载响应中的原文件名。逐条说明见 [`PRIVACY.md`](PRIVACY.md)。
...
测试直接读 `extension/content.js` 源码求值，**没有构建步骤，也不会出现两份实现漂移**。覆盖格式映射、URL 列表解析与节点转换、导出结果文件名归一、文件名清洗与唯一 ID 拼接、日志复制与来源链接、图片重试、产物下载重试分类、重名处理、树扁平化、空间根定位、手写的 zip 容器、云空间节点归一，以及两份语言文件（key 集合一致、占位符个数一致、代码与 manifest 引用的 key 都存在、界面上没有写死的中文、商店字段不超 Chrome 的字符上限）。

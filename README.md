...
选来源，点「列出文档」（在知识库文档页或文件夹页上，打开面板会自动列出）。来源会跟着当前页面自动切：

- **当前知识库** —— 从你打开的这篇爬到空间根节点，拉出整棵树（需要在 `/wiki/xxx` 页面上）
- **当前文件夹** —— 扫这个文件夹往下的全部内容（需要在 `/drive/folder/xxx` 页面上）。**别人分享给你的文件夹走这个** —— 它不在你自己空间的根下，下面那个来源扫不到
- **我的云空间** —— 你自己空间根目录往下的全部内容，任何页面都能扫

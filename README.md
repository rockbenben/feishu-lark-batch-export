...
- **URL列表 TXT** —— 选择一个每行一个 URL 的 `.txt` 文件。链接必须是当前打开的飞书 / Lark 站点，支持知识库、新旧版文档、表格、多维表格和附件链接；空行忽略，重复链接按原顺序保留
...
通过 `file` URL 直接下载附件时，链接里没有原文件名，当前以唯一 ID 作为文件名；从知识库、文件夹或云空间列出的附件仍保留原文件名。
...
最终产物和附件下载单次最长等待 120 秒；网络错误、HTTP 408 / 429 / 5xx 会自动重试，最多重试两次：默认间隔 1 秒，响应带 `Retry-After` 时按服务端要求等待（封顶 30 秒）。401 / 403 / 404 等确定性失败立即进入失败列表，不会重复请求；创建导出任务和进度轮询保持原有逻辑。最终失败只记录一条日志，并附上文档或附件的来源链接。
...
**一并保存图片**跟格式选择相互独立：只要某篇最终产出的是 `.md`，就把图片抓到 `assets/<文档名>/001.png` 并把链接改成相对路径。所以选「自动」时照样带图。docx / pdf / xlsx 是二进制，图片已经内嵌在文件里，这个开关对它们无意义。图片单次请求 15 秒超时，失败后间隔 1 秒重试，最多重试两次；三次都失败才在日志里记录一次错误和图片链接，Markdown 保留原链接，不会把链接改瞎。同批次同名文档（重复链接就会造成这种情况）的图片目录自动追加 `-2`、`-3` 后缀，避免 zip 内图片互相覆盖。

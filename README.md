...
最终产物和附件下载单次最长等待 120 秒；网络错误、HTTP 408 / 429 / 5xx 会间隔 1 秒自动重试，最多重试两次。401 / 403 / 404 等确定性失败立即进入失败列表，不会重复请求；创建导出任务和进度轮询保持原有逻辑。最终失败只记录一条日志，并附上文档或附件的来源链接。

面板里有进度条和按实测均速算的剩余时间，长任务不用干等着猜。

日志区域右上角可以一键复制完整日志，反馈失败链接或排查问题时不用手动选中文字。
...
**一并保存图片**跟格式选择相互独立：只要某篇最终产出的是 `.md`，就把图片抓到 `assets/<文档名>/001.png` 并把链接改成相对路径。所以选「自动」时照样带图。docx / pdf / xlsx 是二进制，图片已经内嵌在文件里，这个开关对它们无意义。图片单次请求 15 秒超时，失败后间隔 1 秒重试，最多重试两次；三次都失败才在日志里记录一次错误和图片链接，Markdown 保留原链接，不会把链接改瞎。

...
- **URL列表 TXT** —— 选择一个每行一个 URL 的 `.txt` 文件。链接必须是当前打开的飞书 / Lark 站点，支持知识库和新旧版文档链接；空行忽略，重复链接按原顺序保留

链接列表只增加一种输入来源：列出后仍进入同一份勾选清单，并复用原来的导出、下载、图片本地化和 zip 流程。普通文档直接从 URL 取得类型和唯一 ID，因此列出时不需要逐篇访问网页；知识库链接仍需调用现有接口把 wiki token 转成实际文档 token。列表中的普通文档暂以唯一 ID 显示，导出成功后，文件名会优先采用飞书导出结果返回的真实标题。原有导出请求会逐篇验证实际读取 / 下载权限；页面不存在或没有权限会在日志中说明，其他文档继续处理。
